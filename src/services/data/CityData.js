...
cityList.forEach((item) => {
  // Give the cities a unique ID.
  item.geoId = item.city + item.rank;

  // Convert the percentage text to a ratio (number).
  item.growth_from_2000_to_2013 = parseFloat(item.growth_from_2000_to_2013.replace("%", "")) / 100;

  // Convert the rank into a number.
  item.rank = parseInt(item.rank);

  // Convert the population into a number.
  item.population = parseInt(item.population);

  // Store the cities
  cityById[item.geoId] = item;
});
